...
当前分支职责、固定运行版本、严格对照配置和最新结果统一记录在
[SWE 实验索引](SWE_EXPERIMENT_INDEX.md)。`main` 保存已经验证的 baseline 和通用稳定性修复；
P1 privileged critic 保留在独立实验分支，在验证结果达到合入标准前不进入主线。
工作文档入口：

- [SWE 实验索引与仓库管理规则](SWE_EXPERIMENT_INDEX.md)
...
当前资源分配（2026-09-15）：P1 使用物理 GPU 0–3，恢复后的 baseline 使用 GPU 4–7。
启动任何新运行前仍须检查对应 GPU 是否空闲，并显式设置 `AGL_GPUS`；不要依赖历史文档中的默认分配。

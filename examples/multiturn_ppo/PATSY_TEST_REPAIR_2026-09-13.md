...
2026-09-13。本报告记录的 Patsy 修复验证只运行 CPU/Docker，没有启动模型或占用 GPU。用户随后批准剔除 9 条纯测试变异任务，正式运行现已启动，见 [全量启动汇报](工作汇报_全量Python_baseline启动_2026-09-13.md)。
...
用户随后明确批准剔除这 9 条并启动全量训练。已生成 `python-full-v6`：6251 train / 470 val，保留 6 条已修复的混合任务；四个完整 epoch 为 784 个 PPO step。合并此前 55 条联网依赖任务，共剔除 64 条，证据与原始行哈希保存在 `logs/swe-approved-exclusions-20260913-02.json`。验证集与 v5 逐字节一致，其他训练行及顺序保持不变。原始数据和旧版本完整保留。不得将本次 6 条参考补丁成功率当作模型成功率。

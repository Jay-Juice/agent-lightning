...
这两题包含 agent 流程限制的影响，不能仅归因于模型。后续离线重放排除新建临时测试文件后，
两条源码补丁仍因 SyntaxError 失败，原成绩不变；详见 [零奖励复核](SWE_ZERO_REWARD_RECHECK_2026-09-10.md)。
...
后续复核确认 32/32 正确代码对照通过、621/621 次调用输入完整；也发现本地单次输出预算
768 tokens 显著小于原部署模板的 12288 tokens，不能将零分归因于模型单一因素。

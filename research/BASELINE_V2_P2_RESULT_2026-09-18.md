...
后续进展：28份固定补丁已全部复核（参考全通过、候选全复现失败），470条真实轨迹的离线契约重放已通过；修复后恢复的固定补丁评分为69/470=14.68%。生产分类修复及重启状态见 `SWE_BASELINE_V2_GRADING_FIX_2026-09-18.md`。下文为最初故障排查快照，原始run保持失败、不改写历史。

...
PREPARED="${1:?Pass the new, prepared diagnostic output directory}"
[[ -f "$PREPARED/inputs.json" && -f "$PREPARED/critic-config.json" ]]
[[ ! -e "$PREPARED/run.exit" && ! -e "$PREPARED/default" && ! -e "$PREPARED/zero" ]]
...
for arm in default zero; do
  ...
  timeout --signal=TERM --kill-after=60 7200 torchrun --standalone --nproc_per_node=4 \
    "$SOURCE/research/critic_fit_diagnostic.py" run --prepared "$PREPARED" --arm "$arm"
done
echo CRITIC_FIT_PAIRED_COMPLETE

...
PROJECT_DIR="$(pwd)"
source "$PROJECT_DIR/examples/alfworld/runtime_env.sh"
...
PPO_COMMAND=(python3 -m arft.main_agent_ppo \
...
# Capture the exact composed configuration before launching this run.
if [[ -n "${CAPO_RESOLVED_CONFIG_PATH:-}" ]]; then
    "${PPO_COMMAND[@]}" --cfg job --resolve > "$CAPO_RESOLVED_CONFIG_PATH"
fi
if [[ "${CAPO_CONFIG_ONLY:-0}" != "1" ]]; then
    "${PPO_COMMAND[@]}"
fi
